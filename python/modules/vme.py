...
def wb(address,byte,serial_port):
 # Ensure byte is in hexadecimal format
    ...
    serial_port.write("wb {:08X} {:02X}\r\n".format(full_address,byte).encode())
...
def wb2(address, byte, serial_port):
    full_address = base_address + address
    # Convert byte to integer
    byte_as_int = int(byte, 16)
    #Copy message to string
    message = "wb {:08x} {:02x}\r\n".format(full_address, byte_as_int)
    # Send string over serial
    print(message)
    serial_port.write("ww hello world hey hey\r\n")
    ...
    response = serial_port.readline().decode().strip()
    # Look for correct string in serial buffer
    if response != "Written: 0x{:02X} at Addr 0x{:08X}".format(byte_as_int, full_address):
        ret = True
    else:
        ret = False
    response = serial_port.readline().decode().strip()
    response = serial_port.readline().decode().strip()
    return ret
#Addres is VME address in hex format
#byte is what has to be sent. It is string in hex format, for example '0x12'
#Target message that has to be sent over serial port is "wb 0xaddress 0xbyte"
    full_address=base_address+address
    #Send string over serial
    serial_port.write("wb {:08x} {:02x}\r\n".format(full_address,byte).encode())
    #serial_port.write(f"wb {full_address:08x} {byte}\r\n".encode())
